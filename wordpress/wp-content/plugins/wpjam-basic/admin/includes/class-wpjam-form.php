<?php
class WPJAM_Form{
	public static $field_tmpls = array();
	
	public static function get_field_value($field, $args=array()){
	...
	// 显示字段
	public static function fields_callback($fields, $args=array()){
		extract(wp_parse_args($args, array(
			'fields_type'	=> 'table',
			'data_type'		=> 'form',
			'field_name'	=> '',		// 合并存储
		...
		switch ($type) {
			...
			case 'mu-text':
				...
				if(!empty($total)){
					for($i=0; $i<$total; $i++) { 
						$item	= ($value[$i])??'';
						$field_html .= '<div class="mu-item">'.self::get_input_field_html($item_type, $name.'[]', $key, $class, $item, $extra).'</div>';
					}
				}else{
					if(is_array($value)){
						foreach($value as $item){
							if(!empty($item)){
								$field_html .= '<div class="mu-item">'.self::get_input_field_html($item_type, $name.'[]', $key, $class, $item, $extra).$del_item_button.$sortable_dashicons.'</div>';
							}
						}
					}

					$field_html .= '<div class="mu-item">'.self::get_input_field_html($item_type, $name.'[]', $key, $class, '', $extra).' <a class="wpjam-mu-text button">添加选项</a></div>';
	...
	public static function get_mu_fields_html($name, $fields, $i, $value=array()){
		$field_html		= '';
	...
	public static function validate_fields_value($fields, $value=array()){
		$data = array();
	...
	public static function column_callback($column_name, $args=array()){
		extract(wp_parse_args($args, array(
			'id'			=> 0,
			'field'			=> '',
			'data_type'		=> 'form',
