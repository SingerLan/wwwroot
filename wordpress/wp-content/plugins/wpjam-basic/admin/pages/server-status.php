<?php
add_filter('wpjam_server_status_dashboard_widgets', function(){
	global $current_tab;

	if($current_tab == 'server'){
		$dashboard_wdigets = array(
			'server'	=> array('title'=>'服务器信息'),
			'version'	=> array('title'=>'服务器版本',	'context'=>'side'),
			'php'		=> array('title'=>'PHP扩展'),
		);
	}elseif($current_tab == 'opcache'){
		$dashboard_wdigets = array(
			'opcache-usage'			=> array('title'=>'OPCache使用率'),
			'opcache-status'		=> array('title'=>'OPCache状态',		'context'=>'side'),
			'opcache-configuration'	=> array('title'=>'OPCache配置信息',	'context'=>'side')
		);
	}elseif($current_tab == 'memcached'){
		$dashboard_wdigets = array(
			'memcached-usage'		=> array('title'=>'Memcached使用率'),
			'memcached-status'		=> array('title'=>'Memcached状态',	'context'=>'side')
		);
	}

	return $dashboard_wdigets;
});
		...
		<tbody>
			...
			<tr>
				...
				<td><?php 
					...
					// $ipdata = wpjam_parse_ip($_SERVER['SERVER_ADDR']);
					// echo '（'.$ipdata['country'];
					// echo ' '.$ipdata['region'];
...
function wpjam_php_dashboard_widget_callback($dashboard, $meta_box){
	?>
	<table class="widefat" style="border:none;">
		<tbody>
			<tr class="alternate">
...
function wpjam_version_dashboard_widget_callback($dashboard, $meta_box){
	?>
	<table class="widefat" style="border:none;">
		<tbody>
			<tr class="alternate">
