...
function wpjam_get_setting($option, $setting_name){
	if(is_string($option)) {
		$option = wpjam_get_option($option);
	}
	...
	if(is_multisite()){
		if(is_network_admin()){
			...
			if(!isset($site_options[$option_name])){
				$site_options[$option_name]	= get_site_option($option_name) ?: [];
			}

			
			$option	= get_option($option_name)?:[];
			$option	= $option + $site_options[$option_name];
...
function wpjam_is_mobile_number($number){
	return preg_match('/^0{0,1}(1[3,5,8][0-9]|14[5,7]|166|17[0,1,6,7,8]|19[8,9])[0-9]{8}$/', $number);
}
