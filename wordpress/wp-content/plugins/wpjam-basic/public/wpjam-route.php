...
add_action('generate_rewrite_rules', function ($wp_rewrite){
	...
	$rules['feed/(feed|rdf|rss|rss2|atom)/?$']	= 'index.php?&feed=$matches[1]';
	$rules['(feed|rdf|rss|rss2|atom)/?$']		= 'index.php?&feed=$matches[1]';

	$rules['api/([^/]+).json?$']				= 'index.php?module=json&action=$matches[1]';

	$wp_rewrite->rules		= array_merge(apply_filters('wpjam_rewrite_rules', $rules), $wp_rewrite->rules);
