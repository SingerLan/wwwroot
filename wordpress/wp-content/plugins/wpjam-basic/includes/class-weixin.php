...
class WEIXIN{
	...
	public function get_oauth_access_token($code){
		$user_oauth	= wp_cache_get($code, 'weixin_'.$this->appid.'_code');
		...
		if($user_oauth === false) {
			...
			wp_cache_set($code, $user_oauth, 'weixin_'.$this->appid.'_code', MINUTE_IN_SECONDS*5);	// 防止同个 code 多次请求
		}
	...
	public function get_material($media_id, $type='image', $force=false){
		...
		if($type=='image' || $type=='thumb'){
			...
			$material = wp_cache_get( $media_id, 'weixin_'.$this->appid.'_material');
			if($material === false || $force){
				...
				wp_cache_set($media_id, $material, 'weixin_'.$this->appid.'_material', DAY_IN_SECONDS);
			}
			return $material;
		}elseif($type == 'video'){
			$response	= $this->http_request($url, array(
	...
	public function del_material($media_id){
		wp_cache_delete($media_id, 'weixin_'.$this->appid.'_material');
