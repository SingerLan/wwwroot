...
class WEAPP{
	...
	public function jscode2session($code){
		$session_data	= wp_cache_get($code, 'weapp_'.$this->appid.'_code');
		...
		wp_cache_set($code, $session_data, 'weapp_'.$this->appid.'_code', MINUTE_IN_SECONDS*15);

		wp_cache_set($session_data['openid'], $session_data['session_key'], 'weapp_'.$this->appid.'_session_key', DAY_IN_SECONDS);
	...
	public function get_session_key_by_openid($openid){
		...
		$session_key	= wp_cache_get($openid, 'weapp_'.$this->appid.'_session_key');
	...
	public function get_access_token($force=false){
		if($this->component_blog_id){
			...
			$access_token = wp_cache_get($this->appid.'_access_token', 'weapp');
			...
			if ($access_token === false || $force) {
				...
				if(is_wp_error($response)){
					...
					wp_cache_set($this->appid.'_access_token', $access_token, 'weapp', $expires_in-600);
				}
			}
		}
	...
	private function http_request($url, $args=array()){
		...
		if(is_wp_error($response)){
			...
			if($errcode == '40001' || $errcode == '40014' || $errcode == '42001'){
				...
				wp_cache_delete($this->appid.'_access_token', 'weapp');
