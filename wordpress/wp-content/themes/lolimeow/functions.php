  ...
  博主与以下 <span class="badge badge-danger">' . count($bookmarks) . '</span> 位大佬存在 <span class="badge badge-default">Jian情</span> 排名不分先后
        ...
        foreach ($bookmarks as $bookmark) {
			...
			<div class="userItem-name"><a class="link link--primary" href="' . $bookmark->link_url . '" target="_blank" >' . $bookmark->link_name . '</a></div></div></div></div>';
				 ...
				 $output .= '<div class="userItem"><div class="userItem--inner"><div class="userItem-content"><img alt="' . $bookmark->link_description . '" src="'. $bookmark->link_image .'" title="' . $bookmark->link_description . '" width="80" height="80" class="author avatar img-raised">
			<div class="userItem-name"><a class="link link--primary" href="' . $bookmark->link_url . '" target="_blank" >' . $bookmark->link_name . '</a></div></div></div></div>';
		 }
            else{$output .= '<div class="userItem"><div class="userItem--inner"><div class="userItem-content"><img alt="' . $bookmark->link_description . '" src="'.get_template_directory_uri().'/assets/images/avatar.jpg" title="' . $bookmark->link_description . '" width="80" height="80" class="author avatar img-raised">
			<div class="userItem-name"><a class="link link--primary" href="' . $bookmark->link_url . '" target="_blank" >' . $bookmark->link_name . '</a></div></div></div></div>';
		}
...
function get_the_link_items_index($id = null){
    ...
    $output    = '';
    if (!empty($bookmarks)) {
        $output .= '<div class="container wow fadeInUp animated"><div class="row"><div class="col-lg-12 col-md-12"><div class="card mt40"><div class="card-body"><a href="https://ttm.im/links" class="btn btn-danger btn-sm justify-content-center" target="_blank" data-toggle="tooltip" data-original-title="点击进入申请友情链接"><span>友情小站</span><span class="badge badge-white">' . count($bookmarks) . '</span></a>';
        foreach ($bookmarks as $bookmark) {
