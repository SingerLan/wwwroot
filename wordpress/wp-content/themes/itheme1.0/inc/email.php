...
class LbMail {
	...
	function __construct(){
		...
		add_filter( 'wp_mail_from', array( $this,'barley_wp_mail_from' ) );
		add_action(	'comment_form', array( $this,'barley_comment_add_checkbox' ) );
		add_action(	'comment_post', array( $this,'barley_comment_mail_notify' ) );
		add_action(	'comment_post', array( $this,'barley_comment_send', 19, 2 ));

	}
	public function barley_mail_smtp( $phpmailer ) {
		$Setting=new LbSetting();
		$phpmailer->IsSMTP();
	...
	public function barley_wp_mail_from( $original_email_address ) {
		return barley_get_setting('smtp-account');
	}
	public function barley_comment_add_checkbox() {
		echo '<label for="comment_mail_notify" class="comment_mail_notify"><input type="checkbox" name="comment_mail_notify" id="comment_mail_notify" value="comment_mail_notify" checked="checked"/>有人回复时邮件通知我</label>';
	}
	
	public function barley_comment_mail_notify($comment_id) {
